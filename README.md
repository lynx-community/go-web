...
| `mode`             | `'linked' \| 'preview' \| 'source' \| 'ultra'` | Overall layout mode (default: `'linked'`). `'linked'` shows code + preview together; `'preview'` shows preview only; `'source'` shows code only; `'ultra'` is an absolutely chromeless full-viewport `<lynx-view>` (requires a web bundle). |
