  ...
  (
    ...
    const setSecondSize = useCallback(
      (px: number) => {
        ...
        if (!el) return;
        const containerInner = getContainerInner(
    ...
    useLayoutEffect(() => {
      ...
      if (vertical) {
        if (collapsed) {
          ...
          setSecondSize(
            containerInner
              ? Math.round((containerInner - DIVIDER_SIZE) / 2)
              : COLLAPSED_SIZE,
          );
        }
      } else {
    ...
    useLayoutEffect(() => {
      ...
      if (collapsed) {
        ...
        setSecondSize(
          Math.max(containerInner - DIVIDER_SIZE, 0) || COLLAPSED_SIZE,
        );
      } else if (vertical) {
        const containerInner = getContainerInner(containerRef.current, true);
        setSecondSize(
      ...
      (previewSize: number) => {
        ...
        if (!collapsed && !firstCollapsed && containerInner > 0) {
          const maxPreview = Math.max(
            containerInner - CODE_SOFT_MIN - DIVIDER_SIZE,
            0,
          );
    ...
    const handlePointerDown = useCallback(
      (e: React.PointerEvent) => {
        ...
        const onPointerMove = (ev: PointerEvent) => {
          ...
          const max =
            containerInner > 0 ? containerInner - DIVIDER_SIZE : newSize;
    ...
    return (
      ...
      >
        ...
        {vertical ? (
          <div role="separator" {...dividerProps} aria-orientation="horizontal">
            <IconHandle
              style={{
                fontSize: '10px',
          ...
          <div role="separator" {...dividerProps} aria-orientation="vertical">
